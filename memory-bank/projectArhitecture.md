# Архитектура проекта Knowledge Base (Актуализировано)
...
Проект разворачивается на production-сервере как набор Docker-контейнеров, управляемых через Docker Compose.

Используется подход **"pre-built image"**:
1.  Docker-образ приложения (`app` + `worker`) собирается и тестируется в CI/CD или на локальной машине разработчика.
2.  Готовый образ загружается в Docker Registry.
3.  Production-сервер **скачивает** (`pull`) уже готовый образ и запускает его. Сборка на продакшен-сервере не производится.
